...
public class AuthService {
    ...
    // Set this to whatever redirect URI you use in the authorization request (frontend or backend)
    // Example: "http://localhost:3000/auth/callback"  OR "http://localhost:8080/api/auth/github/callback"
    ...
    /**
     * Resolve email by checking profile first; if null, call /user/emails and pick primary & verified.
     */
    ...
    /**
     * Upsert user using providerId first; if not found then try email (to avoid duplicates).
     * Also fills sensible defaults so DB fields aren't null for optional profile fields.
     */
    private User createOrUpdateGithubUser(Map<String, Object> githubUser, String accessToken) {
        String githubId = githubUser != null && githubUser.get("id") != null ? String.valueOf(githubUser.get("id")) : null;

        Optional<User> optionalByProvider = Optional.empty();
        if (githubId != null) optionalByProvider = userRepository.findByProviderId(githubId);
        ...
        if (email == null || email.isBlank()) {
            // LAST resort fallback - avoid failing entire flow; keep a recognizable fallback
        ...
        user.setLocation(githubUser != null && githubUser.get("location") != null ? (String) githubUser.get("location") : "");
    ...
    private User createOrUpdateGoogleUser(Map<String, Object> googleUser) {
        ...
        user.setAvatarUrl(googleUser != null ? (String) googleUser.get("picture") : "https://ui-avatars.com/api/?name=" + urlEncode(name));
        user.setUpdatedAt(LocalDateTime.now());
        if (user.getCreatedAt() == null) user.setCreatedAt(LocalDateTime.now());
